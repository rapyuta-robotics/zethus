...
THREE.TransformControls = function ( camera, domElement ) {
  ...
  // Transalate visibility property
  defineProperty('showX', false);
  defineProperty('showY', false);
  defineProperty('showZ', false);
  defineProperty('showXYZ', false);

  // Translate Plane visibility property
  defineProperty('showXY', false);
  defineProperty('showYZ', false);
  defineProperty('showXZ', false);

  // Rotate visibility property
  defineProperty('showRE', false);
  defineProperty('showRX', false);
  defineProperty('showRY', false);
  defineProperty('showRZ', false);
  defineProperty('showRXYZE', false);

  defineProperty('autoScale', false);
  ...
  this.pointerHover = function ( pointer ) {
    ...
    if (this.mode === 'combined') {
      raycastObj = raycastObj.concat(_gizmo.picker['translate'].children);
      raycastObj = raycastObj.concat(_gizmo.picker['rotate'].children);
    } else {
      raycastObj = raycastObj.concat(_gizmo.picker[ this.mode ].children);
    }
  ...
  this.pointerDown = function ( pointer ) {
    ...
    if ( ( pointer.button === 0 || pointer.button === undefined ) && this.axis !== null ) {
      ...
      if ( planeIntersect ) {
        ...
        } else if ( this.axis === 'RE' || this.axis === 'RXYZE' || this.axis === 'XYZ' ) {
  ...
  this.pointerMove = function ( pointer ) {
    ...
    } else if ( axis === 'RE' || axis === 'RXYZE' || axis === 'XYZ' ) {
    ...
    if ( mode === 'rotate' || mode === 'combined') {
      ...
      // Apply rotate
      if ( space === 'local' && axis !== 'RE' && axis !== 'RXYZE' ) {
...
THREE.TransformControlsGizmo = function () {
  ...
  this.updateMatrixWorld = function () {
    ...
    this.gizmo[ "translate" ].visible = this.mode === "translate" || this.mode === 'combined';
    this.gizmo[ "rotate" ].visible = this.mode === "rotate" || this.mode === 'combined';
    this.gizmo[ "scale" ].visible = this.mode === "scale";

    this.helper[ "translate" ].visible = this.mode === "translate" || this.mode === 'combined';
    this.helper[ "rotate" ].visible = this.mode === "rotate" || this.mode === 'combined';
    ...
    for ( var i = 0; i < handles.length; i ++ ) {
      ...
      if ( (this.mode === 'translate' || this.mode === 'scale' || this.mode === 'combined') && handle.name.indexOf('R') !== 1 ) {
        ...
        if ( handle.name.search( 'X' ) !== - 1 && handle.name.search('R') === -1) {
        ...
        if ( handle.name.search( 'Y' ) !== - 1 && handle.name.search('R') === -1) {
        ...
        if ( handle.name.search( 'Z' ) !== - 1 && handle.name.search('R') === -1) {
